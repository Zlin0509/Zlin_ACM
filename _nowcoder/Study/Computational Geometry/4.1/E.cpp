...
inline T p2line(const Point &u, const Point &v, const Point &w) {
    Line l(u, v - u);
    return l.dis(w);
}
...
struct Convex : Polygon {
    T rotcaliper() const {
        T ans = INF;
        const auto &p = this->p;
        const auto area = [](const Point &u, const Point &v, const Point &w) { return (u - w) ^ (v - w); };
        for (size_t i = 0, j = 1; i < p.size(); i++) {
            size_t nxti = this->nxt(i);
            while (area(p[this->nxt(j)], p[i], p[nxti]) >= area(p[j], p[i], p[nxti])) {
                j = this->nxt(j);
            }
            ans = min(ans, p2line(p[i], p[nxti], p[j]));
        }
        return ans;
    }
};
...
inline void Zlin(int n) {
    ...
    cout << fixed << setprecision(10) << poly.rotcaliper() << endl;
}
